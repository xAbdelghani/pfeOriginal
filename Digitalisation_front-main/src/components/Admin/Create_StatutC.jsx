...
const Create_StatutC = () => {
    const [StatutCid, setId] = useState('');
    const [libellec, setLibellec] = useState(''); 
   

    const [statutCs, setUsers] = useState([]);
    ...
    const validate = () => {
        const newErrors = {};
        if (!libellec) newErrors.libellec = "* Ce champ est obligatoire";
    ...
    async function save(event) {
        ...
        try {
            await axiosInstance.post("http://localhost:8080/api/v1/statutC/save", {
                        ...
                        <div className="card-body">
                            <form>
                                ...
                                {errors.libelle && <div style={{ color: 'red' }}>{errors.libelle}</div>}
                                    ...
                                    <input type="text" className="form-control" value={libellec} onChange={(event) => setLibellec(event.target.value)} placeholder="Entrez Libelle" />
                                </div>
                                <br />
