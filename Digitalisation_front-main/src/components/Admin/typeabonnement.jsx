...
const Type_Abonnements = () => {
    const [typeabonnements, setTypeAbonnements] = useState([]);
    const [typeabonnementIdToDelete, setTypeAbonnementIdToDelete] = useState(null);
    ...
    function handleEditAbonnement(typeabonnement) {
        navigate("/admin/EditT", { state: { typeabonnement } });
    }

    async function deleteAbonnement() {
        setAlertVisible(true);
        await axiosInstance.delete("http://localhost:8080/api/v1/typeabonnement/delete/" + typeabonnementIdToDelete);
    ...
    async function modell(typeabonnementId) {
        setModalVisible (true);
        setTypeAbonnementIdToDelete(typeabonnementId);
    }
    ...
    const columns = [
            ...
            {
                title:<span style={{ color: '#607AD6' }}>Durée</span>,
                dataIndex: 'duree',
                key: 'duree',
            ...
            {
                title:<span style={{ color: '#607AD6' }}>Unité</span>,
                dataIndex: 'unite',
                key: 'unite',
            ...
            },
        {
            ...
            render: (text, record) => (
                ...
                <Modal
                    title="Confirmation"
                    open={modalVisible}
                    onOk={() =>{deleteAbonnement(record.id);setModalVisible(false)
                    }} 
    ...
    return (
        <>
            ...
            <div className="container">
                ...
                <Table
    columns={columns}
    dataSource={typeabonnements ? typeabonnements.map((typeabonnement) => ({ ...typeabonnement, key: typeabonnement.id })) : []}
...
export default Type_Abonnements;
