...
const Offres = () => {
    ...
    const [offreIdToDelete, setOffreIdToDelete] = useState(null);
    const [offreId, setId] = useState('');
    ...
    async function loadOffres() {
        const result = await axiosInstance.get("http://localhost:8080/api/v1/offre/getall");
        setOffres(result.data);
    }

    function handleEditOffre(offre) {
        navigate("/admin/Editof", { state: { offre } });
    }

    async function deleteOffre() {
        setAlertVisible(true);
        await axiosInstance.delete("http://localhost:8080/api/v1/offre/delete/" + offreIdToDelete);
    ...
    async function modell(offreId) {
        setModalVisible (true);
        setOffreIdToDelete(offreId);
    }
    ...
    const columns = [
           ...
           
        {
            ...
            render: (text, record) => (
                ...
                <Space size="middle">
                <Button  onClick={() => handleEditOffre(record)} style={{ color: '#607AD6' , display: 'inline-flex', alignItems: 'center' }}>
                <EditOutlined />
                  Modifier
                </Button>
                <Button onClick={() => modell(record.id)} style={{ color: '#607AD6', display: 'inline-flex', alignItems: 'center' }}>
                  <DeleteOutlined />
                  Supprimer
                </Button>
                ...
                <Modal
                    title="Confirmation"
                    open={modalVisible}
                    onOk={() =>{deleteOffre(record.id);setModalVisible(false)
                    }} 
    ...
    return (
        <>
            ...
            <div className="container">
                ...
                <Table
    columns={columns}
    dataSource={offres ? offres.map((offre) => ({ ...offre, key: offre.id })) : []}
