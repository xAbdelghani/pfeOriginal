...
const StatusC = () => {
    ...
    const [statutCIdToDelete, setStatutCIdToDelete] = useState(null);
    const [statutCId, setId] = useState('');
    ...
    function handleEditstatutC(statutC) {
        navigate("/admin/EditStC", { state: { statutC } });
    }

    async function deletestatutC() {
        setAlertVisible(true);
        await axiosInstance.delete("http://localhost:8080/api/v1/statutC/delete/" + statutCIdToDelete);
    ...
    async function modell(statutCId) {
        setModalVisible (true);
        setStatutCIdToDelete(statutCId);
    }
            ...
            render: (text, record) => (
                ...
                <Space size="middle">
                    ...
                    <Tooltip title="Modifier">
                        <span className="compagnie-bg" onClick={() => handleEditstatutC(record)}>
                ...
                <Modal
                    title="Confirmation"
                    open={modalVisible}
                    onOk={() =>{deletestatutC(record.id);setModalVisible(false)
                    }} 
    ...
    return (
        <>
            ...
            <div className="container">
                ...
                <Table
    columns={columns}
    dataSource={statutCs ? statutCs.map((statutC) => ({ ...statutC, key: statutC.id })) : []}
