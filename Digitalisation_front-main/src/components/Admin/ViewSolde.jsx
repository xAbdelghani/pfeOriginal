...
export default function ViewSolde() {
  ...
  const loadSolde = async () => {
    try { 
      const result = await axiosInstance.get(`http://localhost:8080/api/v1/solde/${soldeId}`);
      setSolde(result.data);
  ...
  const handleEdit = () => {
    setEditing(true);
    form.setFieldsValue(solde); // Update form values when editing
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const updatedSolde = { ...solde, ...values };
      await axiosInstance.put(`http://localhost:8080/api/v1/solde/${soldeId}`, updatedSolde, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
      setSolde(updatedSolde);
      setEditing(false);
  ...
  const { raison_social, solde: montant, date_Abonnement,libellep, type, devise, statut, statutHistorique } = solde;
    ...
    <div className="container">
      ...
      <div className="row">
        <div className="col-md-10 offset-md-1 border rounded p-4 mt-2 shadow">
          <h2 className="text-center m-4">Détails Prépayement</h2>
          <div className="card">
            <div className="card-header" style={{ backgroundColor: 'rgba(96, 122, 214, 0.90)', color: 'white' }}>
              <h5><strong> Prépayement </strong></h5>
            </div>
            <div className="card-body">
              ...
              <ul className="list-group list-group-flush">
                <li>
                  ...
                  <b>Coùt : </b>{montant} {devise}<br /><br />
                  <b>Date d'abonnement : </b>{date_Abonnement || 'N/A'}<br /><br />
                  <b>Type : </b>{type}<br /><br />
          ...
          <br />
          <Link className="btn btn-primary my-2" style={{ backgroundColor: 'rgba(96, 122, 214, 0.99)',marginLeft:"45%" }} to={"/solde"}>
            Retour 
          </Link>
        </div>
      </div>
