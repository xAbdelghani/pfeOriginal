...
const Type_Attestations = () => {
    const [typeattestations, setTypeattestations] = useState([]);
    const [typeattestationIdToDelete, setTypeattestationIdToDelete] = useState(null);
    ...
    async function loadtypeattestations() {
        const result = await axiosInstance.get("http://localhost:8080/api/v1/typeattestation/getall");
        setTypeattestations(result.data);
    }

    function handleEdittypeattestation(typeattestation) {
        navigate("/admin/EditTyp", { state: { typeattestation } });
    }

    async function deletetypeattestation() {
        setAlertVisible(true);
        await axiosInstance.delete("http://localhost:8080/api/v1/typeattestation/delete/" + typeattestationIdToDelete);
    ...
    async function modell(typeattestationId) {
        setModalVisible (true);
        setTypeattestationIdToDelete(typeattestationId);
    }
    ...
    const columns = [
            {
                title: <span style={{ color: '#607AD6' }}>Description</span>,
                dataIndex: 'libelle',
                key: 'libelle',
            ...
            {
                title:<span style={{ color: '#607AD6' }}>Prix Unitaire</span>,
            ...
            {
                title:<span style={{ color: '#607AD6' }}>Devise</span>,
                dataIndex: 'devise',
                key: 'devise',
                ...getColumnSearchProps( 'unite'),
            },
        {
            ...
            render: (text, record) => (
                ...
                <Space size="middle">
                    ...
                    <Tooltip title="Modifier">
                        <span className="compagnie-bg" onClick={() => handleEdittypeattestation(record)}>
                ...
                <Modal
                    title="Confirmation"
                    open={modalVisible}
                    onOk={() =>{deletetypeattestation(record.id);setModalVisible(false)
                    }} 
    ...
    return (
        <>
            ...
            <div className="container">
                ...
                <Table
    columns={columns}
    dataSource={typeattestations ? typeattestations.map((typeattestation) => ({ ...typeattestation, key: typeattestation.id })) : []}
...
export default Type_Attestations;
